...
const sections = [
  {
    ...
    title: "ENGINEERED\nPRECISION",
    sub: "German Mastery",
    text: "Every vehicle is a testament to German engineering philosophy — sourced only from the pinnacle of performance heritage, where aesthetics and mechanics converge into something irreducible.",
  ...
  },
  {
    ...
    title: "GLOBAL\nLOGISTICS",
    sub: "Seamless Export",
    text: "Our network spans continents. Every transition — legal, mechanical, logistical — is handled with the absolute discretion and precision that defines our reputation.",
  ...
  },
  {
    ...
    title: "PRESTIGE\nSERVICE",
    sub: "Beyond the Sale",
    text: "24/7 technical concierge tailored for the most demanding collectors. We don't close when you drive away — we remain your mechanical partner for life.",
  ...
  },
  {
    ...
    text: "Sport-inspired luxury distilled into our defining collection. Each unit hand-selected, each detail intentional. Available for worldwide delivery today.",
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      gsap.utils.toArray('.ab-row').forEach((row) => {
        ...
        const btn       = row.querySelector('.ab-btn');

        const headerText = heading.getAttribute('data-text');
        const paraText   = para.getAttribute('data-text');

        gsap.set([heading, para], { text: '' });
        gsap.set([indexEl, subEl, rule, btn], { opacity: 0, y: 20 });
        ...
        tl.to(img,    { opacity: 1, x: 0, duration: 1.0, ease: 'power3.out' })
          ...
          .to(heading,{ text: { value: headerText }, duration: 0.7, ease: 'none' }, '-=0.2')
          .to(rule,   { opacity: 1, y: 0, duration: 0.4, ease: 'power2.out' }, '-=0.1')
          .to(para,   { text: { value: paraText }, duration: 0.8, ease: 'none' }, '-=0.1')
          .to(btn,    { opacity: 1, y: 0, duration: 0.5, ease: 'power2.out' }, '-=0.2');
      });
    }, containerRef);
    ...
    <>
      <style>{`
        ...
        @media (max-width: 800px) {
          .ab-row { grid-template-columns: 1fr; direction: ltr; gap: 40px; padding: 48px 0; }
          .ab-row--reverse > * { direction: ltr; }
        }
        ...
        .ab-btn:hover::before { transform: translateX(0); }
        .ab-btn:hover { color: #060606; border-color: #c9a84c; }
      ...
      <section className="ab-section" ref={containerRef}>
        <div className="ab-inner">
          ...
          {sections.map((item, i) => (
            <div
              key={i}
              className={`ab-row${i % 2 !== 0 ? ' ab-row--reverse' : ''}`}
            >
              {/* Image */}
